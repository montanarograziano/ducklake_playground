...
``event_date`` (``pa.date32``, NOT NULL) as the partition column. Rows are emitted in
event_date-sorted order so the Parquet writer only ever has 1-2 partitions open at once.
...
def _arrow_type(col: ColumnDef) -> pa.DataType:
    ...
    if col.type == "varchar":
        # Dict-encoded; both DuckLake/Parquet and Delta honor this on write.
...
class GeneratorSpec:
    ...
    memory at small scales because each open partition holds a multi-MB write buffer.

    Attributes:
        schema_config: The user-defined columns (id and event_date are always added).
        total_rows: Total rows to generate across the whole stream.
        chunk_size: Rows per yielded ``pa.RecordBatch``. Bounds generator memory.
        seed: Base RNG seed; per-chunk seeds are derived via ``SeedSequence.spawn``.
        date_start: Inclusive start of the ``event_date`` range (partition cardinality lower bound).
        date_end: Inclusive end of the ``event_date`` range.
    """
...
class StreamingGenerator:
    ...
    bounded to 1-2 partitions at any time, regardless of partition cardinality.
    """

    def __init__(self, spec: GeneratorSpec) -> None:
        """Initialize from a ``GeneratorSpec``."""
        self.spec = spec
    ...
    def arrow_reader(self) -> pa.RecordBatchReader:
        ...
        Pass this directly to ``DuckDB.register(...)`` and then ``INSERT INTO ... SELECT * FROM
        <name>`` for true zero-copy streaming writes.
        """
    ...
    def iter_merge_batches(self, overlap_ratio: float) -> Iterator[pa.RecordBatch]:
        ...
        Args:
            overlap_ratio: Fraction in ``[0, 1]`` of rows that reuse base IDs (UPDATE);
                the remainder are new IDs (INSERT).
        """
    ...
    def _chunk(self, n: int, id_offset: int, rng: np.random.Generator) -> pa.RecordBatch:
        ...
        # in stream order. Bounds the Parquet writer to 1-2 open partitions.
        n_partitions = self._date_span_days
